...
const Dashboard = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    const filteredHourlyData = hourUsageStatistic
      ...
      .reduce((acc, item) => {
        const existingHour = acc.find(
          (h) => h.startHour === item.startHour && h.endHour === item.endHour
        );
        if (existingHour) {
          ...
          acc.push({
            ...item,
            hour: `${String(item.startHour).padStart(2, "0")}:00 - ${String(
              item.endHour
            ).padStart(2, "0")}:00`,
          });
        }
    ...
    const filteredStationData = stationUsageStatistic
      ...
      .reduce((acc, item) => {
        const existingStation = acc.find(
          (s) => s.stationName === item.stationName
        );
        if (existingStation) {
  ...
  const totalEntries = stationData.reduce(
    (sum, station) => sum + station.entryCount,
    0
  );
  const totalExits = stationData.reduce(
    (sum, station) => sum + station.exitCount,
    0
  );
  const totalTickets = ticketData.reduce(
    (sum, ticket) => sum + ticket.usageCount,
    0
  );
  const peakHour =
    hourlyData.length > 0
  ...
  const StatisticCard = ({ title, value, prefix, bgColor }: any) => (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div style={{ fontSize: "2em", color: bgColor }}>{prefix}</div>
      </div>
    </Card>
  );

  const hasData =
    hourlyData.length > 0 || stationData.length > 0 || ticketData.length > 0;
  const noDataMessage = (
    <div className="text-center p-10 text-[#999] text-base">
      Không có dữ liệu cho khoảng thời gian {dateRange[0].format("DD/MM/YYYY")}{" "}
          ...
          <Row gutter={[24, 24]} style={{ marginBottom: "24px" }}>
            ...
            <Col xs={24} sm={12} md={6}>
              <StatisticCard
                ...
                value={
                  peakHour
                    ? `${String(peakHour.startHour).padStart(
                        2,
                        "0"
                      )}:00 - ${String(peakHour.endHour).padStart(2, "0")}:00`
                    : "Không có dữ liệu"
                }
              ...
              <Col xs={24}>
                <Card
                  title={
                    ...
                    >
                      Thống Kê Theo Giờ: {dateRange[0].format("DD/MM/YYYY")} -{" "}
                      {dateRange[1].format("DD/MM/YYYY")}
                    </div>
                  }
                  className="!shadow-[0_1px_2px_0_rgba(0,0,0,0.03),0_1px_6px_-1px_rgba(0,0,0,0.02),0_2px_4px_0_rgba(0,0,0,0.02)]"
              ...
              <Col xs={24} lg={14}>
                <Card
                  title={
                    ...
                    >
                      Thống Kê Theo Ga: {dateRange[0].format("DD/MM/YYYY")} -{" "}
                      {dateRange[1].format("DD/MM/YYYY")}
                    </div>
                  }
                  className="!shadow-[0_1px_2px_0_rgba(0,0,0,0.03),0_1px_6px_-1px_rgba(0,0,0,0.02),0_2px_4px_0_rgba(0,0,0,0.02)]"
              ...
              <Col xs={24} lg={stationData.length > 0 ? 10 : 24}>
                <Card
                  title={
                    ...
                    >
                      Thống Kê Theo Loại Vé: {dateRange[0].format("DD/MM/YYYY")}{" "}
                      - {dateRange[1].format("DD/MM/YYYY")}
                    </div>
                  }
                  className="!shadow-[0_1px_2px_0_rgba(0,0,0,0.03),0_1px_6px_-1px_rgba(0,0,0,0.02),0_2px_4px_0_rgba(0,0,0,0.02)]"
