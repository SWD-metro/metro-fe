...
export const apiCreateStation = async (station: StationsRequest): Promise<ApiResponse<Station> | null> => {
  try {
...
export const apiGetStationByName = async (stationName: string): Promise<ApiResponse<Station[]> | null> => {
  try {
    const res = await http.get(`${API_PATH.STATIONS}/search?name=${stationName}`);
    return res.data as ApiResponse<Station[]>;
  } catch {
    return null;
  }
...
export const apiUpdateStationStatus = async (id: number, status: Status): Promise<ApiResponse<Station> | null> => {
  try {
    const res = await http.post(`${API_PATH.STATIONS}/status/${id}?status=${status}`);
    return res.data as ApiResponse<Station>;
  } catch {
    return null;
  }
